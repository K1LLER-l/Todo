...
from django.db.models import Prefetch # Para eficiencia
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Prefetch para evitar múltiples consultas a la BD dentro del bucle
        favorites_prefetch = Prefetch('favoritos', queryset=Favorito.objects.filter(precio_minimo_deseado__gt=0))
        
        # Obtenemos solo los usuarios que tienen FAVORITOS CON ALERTAS configuradas
        ...
        for user in users_with_alerts:
            ...
            # Recorremos SOLO los favoritos de este usuario
            ...
            # 4. ENVIAR UN SOLO CORREO SI HAY ALERTAS
            if alerts_detected:
                asunto = f"🔥 ¡Ahorra en {len(alerts_detected)} productos favoritos!"
                
                # Contexto para el template (enviamos la lista completa)
                context = {
